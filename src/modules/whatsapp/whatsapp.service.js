...
export const sendMessage = async (tenantId, toPhone, payloadOrText) => {
  ...
  // Enqueue outbox delivery via pg-boss (Postgres-backed, always available)
  try {
    await mainQueue.add('sendOutbox', { outboxId: outbox.id }, { jobId: outbox.id, attempts: 5, backoff: { type: 'exponential', delay: 5000 } });
  } catch (err) {
